...
public class VarInitChecker implements Visitor {
	...
	public Object visit(Assignment assignment) throws SemanticError {
		...
		if (varLocation instanceof VariableLocation) {
			Symbol varSym = varLocation.getEnclosingScope().lookup(
					((VariableLocation) varLocation).getName(), varLocation);
	...
	private void checkIfExpVarsInited(Expression e) throws SemanticError {
		...
		for (Symbol var : vars)
			if (!this.initedVars.contains(var))
				throw new SemanticError("the local variable " + var.getIdName()
						+ " may not have been initialized", e);
	}
	...
	public Object visit(LocalVariable localVariable) throws SemanticError {
		if (localVariable.hasInitValue()) {
			...
			this.initedVars.add(localVariable.getEnclosingScope().lookup(
					localVariable.getName(), localVariable));
		}
		return null;
	}
	...
	public Object visit(VariableLocation location) throws SemanticError {
		// NO NEED TO IMPLEMENT SINCE EXPRESSIONS ARE HANDLED IN
		// "checkIfExpVarsInited"!!!!!
		return null;
	}
